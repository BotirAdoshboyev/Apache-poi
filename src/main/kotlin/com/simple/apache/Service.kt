...
fun getItems(date: Long, type: Long): List<Dto>{
    ...
    for (i in 0..10){
        val item = Dto(i.toLong(), "Samarqand $i", 20+i.toLong(), type, date)
        res.add(i, item)
    }
    return res
}
...
class Controller {
    ...
    fun post() : ResponseEntity<ByteArray>{
        ...
        var startDate = 1608527600000
        val endDate = 1609927600000
        ...
        typeList.forEach { type ->
            ...
            val xlWS = when(type) {
                ...
                502 -> xlWB.createSheet("IR")
                else -> xlWB.createSheet("LBG")
            }
            var totalInDate = 0
            for (i in dateList.indices) {
                val list = getItems(dateList[i], type.toLong())
                list.forEach {
                ...
                list.forEachIndexed { index, dto ->
                    if (index == 0) {
                        ...
                        headerDateCell.setCellValue(pattern.format(dateList[i]))
                        headerDateCell.cellStyle = boldCellStyle
                    }
                    xlWS.getRow(index+1).createCell(0).setCellValue(dto.name)
                    xlWS.getRow(index+1).createCell(i+1).setCellValue(list[index].total.toString())
                    xlWS.autoSizeColumn(i+1)
                    totalInDate += list[index].total.toInt()
                }
